...
export function applyDevEnvMigrateTool(ctx, caps) {
	...
	const tool = defineTool({
		...
		parameters: {
			...
			destinationPath: {
				type: "string",
				description: "Absolute destination directory on another drive. An empty directory is allowed; existing content requires explicit replacement confirmation.",
			},
			method: {
				type: "string",
		...
		output: {
			schema: {
				...
				properties: {
					...
					destinationBackupPath: { type: "string" },
					destinationHasContent: { type: "boolean" },
					destinationFileCount: { type: "number" },
		...
		async execute(args) {
			if (args.mode === "plan") {
				...
				return {
					...
					destinationBackupPath: plan.destinationBackupPath,
					destinationHasContent: plan.destinationHasContent,
					destinationFileCount: plan.destinationFileCount,
					method: plan.method,
			...
			if (args.mode === "execute") {
				...
				return {
					...
					destinationBackupPath: result.destinationBackupPath,
...
function formatMigrationResult(value) {
	if (value.mode === "plan") {
		const lines = [
			...
			...(value.destinationHasContent ? [
				"- 目标目录现有文件: " + (value.destinationFileCount ?? 0),
				"- 目标内容备份: " + value.destinationBackupPath,
				"- 覆盖提醒: 目标目录已有内容，执行后将由迁移内容替换",
			] : []),
	...
	return [
		...
		...(value.destinationBackupPath ? ["- 目标原内容备份: " + value.destinationBackupPath] : []),
