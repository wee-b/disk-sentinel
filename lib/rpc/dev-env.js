...
import { getManualMigrationRule, resolveManualMigration } from "../dev-env-migration-rules.js";
...
const DEV_ENV_CACHE_VERSION = 12;
...
async function collectEnvPathGroups(envVars) {
	...
	const entries = await mapLimit([...byPath.values()], 3, async (entry) => {
		const measured = await measurePath(entry.path);
		return {
			...entry,
			...measured,
		};
...
async function runDevEnvironmentScan() {
	...
	const envPathGroups = await collectEnvPathGroups(envVars);
	...
	const known = await mapLimit(uniqueDirs, 3, async (item) => {
		...
		const manualRule = getManualMigrationRule(item.id);
		const manualMigration = measured.isLink
			? null
			: await resolveManualMigration(manualRule, item.path);
		return {
			id: item.id,
			label: item.label,
			tool: item.tool,
