 ...
 * - dev-env/scan:  兼容旧客户端的同步开发环境扫描
 * - dev-env/scan/start: 启动后台开发环境扫描
 * - dev-env/scan/status: 轮询文件数、目录数、当前路径与最终结果
 * - dev-env/report: 生成供 AI 决策的开发环境迁移报告
 * - dev-env/migration/pick-directory: 打开 Windows 文件夹选择器
 * - dev-env/migration/plan-manual: 仅对白名单目录生成手动迁移计划
 * - dev-env/migration/execute: 执行一次性迁移计划
 *
...
import { startDevEnvScanJob, devEnvJobSnapshot } from "./dev-env-scan-job.js";
import {
	createManualMigrationPlan,
	executeMigrationPlan,
	writeDevEnvMigrationReport,
} from "../dev-env-migration.js";
...
/**
 * 在 DSH WebServer 上注册 JSON RPC 路由。
 *
 * dsh 0.1.5 的 connection.rpc.handle() 会在 connection 自己的作用域中
 * 读取 webServer；链接插件的作用域不包含该服务，因而路由不会挂载。这里
 * 保留相同的请求信封和浏览器会话校验，直接使用本插件已注入的 webServer。
 */
function registerRpcRoute(ctx, dispatch) {
	// 单元测试与旧宿主可继续提供通用 RPC 桩；实际 DSH Web 走下方显式路由。
	if (!ctx.webServer) {
		return ctx.connection.rpc.handle(CHANNEL, dispatch, { authority: "loopback" });
	}
	return ctx.webServer.register({
		...
		handler: async (req, res) => {
			...
			if (!endpoint || message?.type !== "client-request" || typeof message.rpcId !== "string" || message.method !== endpoint) {
				res.writeHead(400);
...
export function applyRpc(ctx) {
	const dispatch = async (endpoint, payload) => {
			try {
				switch (endpoint) {
					...
					case "dev-env/scan/start": {
						return ok(startDevEnvScanJob(payload?.force === true));
					}
					case "dev-env/scan/status": {
						return ok(devEnvJobSnapshot());
					}
					case "dev-env/report": {
						return ok(await writeDevEnvMigrationReport());
					}
					case "dev-env/migration/pick-directory": {
						return ok(await pickDirectory(ctx, String(payload?.initialPath ?? "")));
					}
					case "dev-env/migration/plan-manual": {
						return ok(await createManualMigrationPlan(
							String(payload?.recipeId ?? ""),
							String(payload?.targetRoot ?? "")
						));
					}
					case "dev-env/migration/execute": {
						return ok(await executeMigrationPlan(String(payload?.planId ?? "")));
					}
					case "scan/start": {
	...
	const dispose = registerRpcRoute(ctx, dispatch);
