...
import { DEV_ENV_CACHE_NAME, measurePath, normKey, scanDevEnvironment } from "./rpc/dev-env.js";
...
async function validatePaths(sourcePath, destinationPath) {
	...
	let destinationHasContent = false;
	try {
		const destinationInfo = await lstat(destination);
		if (!destinationInfo.isDirectory() || destinationInfo.isSymbolicLink()) {
			throw new Error("目标路径已存在，但不是可用的真实目录");
		}
		destinationExists = true;
		destinationHasContent = (await readdir(destination)).length > 0;
	} catch (err) {
		if (err?.code !== "ENOENT") throw err;
	}
	return { source, destination, destinationExists, destinationHasContent };
}
...
async function createMigrationPlan(scan, request) {
	...
	const destinationMeasured = paths.destinationExists
		? await measurePath(paths.destination)
	...
	const plan = {
		...
		destinationBackupPath: paths.destinationHasContent
			? paths.destination + ".disk-sentinel-backup-" + stamp + "-" + planId.slice(0, 8)
			: null,
		destinationExists: paths.destinationExists,
		destinationHasContent: paths.destinationHasContent,
		destinationSizeBytes: destinationMeasured.sizeBytes ?? 0,
		destinationFileCount: destinationMeasured.fileCount ?? 0,
		destinationDirCount: destinationMeasured.dirCount ?? 0,
		method,
		envChanges,
		configChange,
...
async function createManualMigrationPlan(recipeId, targetRoot) {
	...
	if (!capability) {
		const configured = await resolveConfiguredLocation(rule);
		if (configured?.path && normKey(configured.path) !== normKey(source.path)) {
			throw new Error(rule.tool + " 持久配置已指向 " + configured.path +
				"，无需再次迁移。当前页面使用了旧扫描结果，请重新扫描开发环境。");
		}
		throw new Error("目标目录可以使用，但当前未找到可修改的 " + rule.tool +
			" 持久配置。请重新扫描开发环境；如果迁移按钮消失，需先创建或修复对应工具配置文件。");
	}
...
async function executeMigrationPlan(planId) {
	...
	const paths = await validatePaths(plan.sourcePath, plan.destinationPath);
	...
	if (paths.destinationExists !== plan.destinationExists ||
		paths.destinationHasContent !== plan.destinationHasContent) {
		throw new Error("目标目录在确认后发生变化，请重新生成迁移计划");
	}
	if (paths.destinationExists) {
		const currentDestination = await measurePath(paths.destination);
		if (currentDestination.sizeBytes !== plan.destinationSizeBytes ||
			currentDestination.fileCount !== plan.destinationFileCount ||
			currentDestination.dirCount !== plan.destinationDirCount) {
			throw new Error("目标目录内容在确认后发生变化，请重新生成迁移计划");
		}
	}
	...
	if (paths.destinationExists) {
		if (paths.destinationHasContent) {
			await rename(plan.destinationPath, plan.destinationBackupPath);
	...
	try {
		...
		if (destinationBackedUp) {
			await rename(plan.destinationBackupPath, plan.destinationPath).catch(() => {});
		} else if (plan.destinationExists) {
			await mkdir(plan.destinationPath, { recursive: true }).catch(() => {});
		}
		throw err;
	}
	...
	return {
		...
		backupPath,
		destinationBackupPath: destinationBackedUp ? plan.destinationBackupPath : null,
		method: plan.method,
...
function renderDevEnvMigrationReport(scan) {
	...
	for (const item of scan.knownDirs ?? []) {
		...
		const access = item.isMigrated
			? "已迁移"
			: isProtectedMigrationPath(item.path)
			? "禁止迁移"
			: item.manualMigration ? "白名单手动 / AI" : "仅 AI";
		const displayPath = item.isMigrated
			? item.path + " -> " + item.effectivePath
			: item.path;
		lines.push("| " + escapeCell(item.label) + " | " + escapeCell(displayPath) + " | " +
...
async function cleanupPreviousDevEnvArtifacts(dir, keepReportPath) {
	const keepName = keepReportPath ? basename(keepReportPath) : "";
	const entries = await readdir(dir, { withFileTypes: true });
	await Promise.all(entries.map(async (entry) => {
		if (!entry.isFile()) return;
		const isOldReport = /^开发环境迁移分析-\d{8}-\d{6}\.md$/.test(entry.name) && entry.name !== keepName;
		const isScanCache = entry.name === DEV_ENV_CACHE_NAME;
		if (isOldReport || isScanCache) {
...
async function writeDevEnvMigrationReport() {
	...
	await cleanupPreviousDevEnvArtifacts(dir, file);
